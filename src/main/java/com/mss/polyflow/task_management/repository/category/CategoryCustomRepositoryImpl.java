...
public class CategoryCustomRepositoryImpl implements CategoryCustomRepository {
    ...
    private void buildRequiredCommonFilteringSortingQuery(boolean countQuery, SearchFilterQueryParameterDto searchQueryParams, StringBuilder queryBuilder, Map<String, Object> queryParamsTracker) {
        searchQueryParams.setType(null);
        ...
        if(requiredOrdering != null && requiredOrdering.trim().length() > 0) {
            ...
            if(orderCriteria.equals("creationTime") || orderCriteria.equals("modificationTime") || orderCriteria.equals("name")) {
                String orderingDirection = "asc";
                orderCriteria = fromCamelCaseToSnakeCase(orderCriteria);
...
//                queryParamsTracker.put("orderingDirection", orderingDirection);
    ...
    private String fromCamelCaseToSnakeCase(String orderCriteria) {
        StringBuilder updatedResultString = new StringBuilder();
        StringBuilder temp = new StringBuilder();
        int n = orderCriteria.length();
        for(int i = 0; i < n; ++i) {
            if(orderCriteria.charAt(i) >= 'A' && orderCriteria.charAt(i) <= 'Z') {
                updatedResultString.append(temp);
                temp.setLength(0);
            } else {
                temp.append(orderCriteria.charAt(i));
            }
        }
        updatedResultString.append(temp);
